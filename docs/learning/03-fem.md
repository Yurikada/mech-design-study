...
M2は有限要素法の数値モデルと、その評価結果を統合する仕組みを学ぶ。まず梁要素で行列の組み立て・拘束・振動を確認し、次に2次元連続体要素でメッシュ配置、要素形状、近似次数を比較する。M2aの梁FEMをAI支援で実装し、数値結果を以下に掲載した。M2bの純曲げ比較は講義4に実装結果を掲載。図は本プロジェクトの概念図と計算結果で、転載図ではない。
