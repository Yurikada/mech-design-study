...
次の講義：[講義5：曲率が変わる問題でメッシュ収束を確かめる](05-mesh-convergence.md)。確認が十分な項目は反復せず、三次変位場を使う新しい検証ケースへ進む。
...
**問い：** 長方形Q4要素の範囲を0≤x≤ℓとする。純曲げ参照解の節点値を補間するとu_h=−κxy、v_h=κℓx/2となる。γ_xy,h=∂u_h/∂y+∂v_h/∂xを求めると、要素内の全点で0になるか。

<details>
<summary>解答例と解説</summary>

**解答例：** γ_xy,h=κ(ℓ/2−x)。κ≠0なら0になるのはx=ℓ/2だけで、要素全域では0にならない。

**解説：** Q4はxyを含むのでuを再現できる。一方、左右の節点でv(0)=0、v(ℓ)=κℓ²/2となり、Q4でのvの補間はx方向に直線になる。∂u_h/∂y=−κxは位置に応じて変わるが、∂v_h/∂x=κℓ/2は一定なので、せん断ひずみを全域で打ち消せない。これは参照解の節点値を入れた補間能力の検査であり、荷重から求解したFEM変位そのものとは区別する。この不要なせん断成分が、曲げに対して余分なエネルギーを与える仕組みを次に考える。長方形・一定曲率の設定に限った導出である。

> γ_xy,h = −κx + κℓ/2 = κ(ℓ/2−x)
