...
GitHubのファイル画面ではHTMLはソース表示になる。clone／ダウンロード後、`docs/learning/01-lecture.html` をブラウザで開く。HTMLと `assets` フォルダを一緒に置く。外部フォント・CDN・API・Webサーバーは不要。将来GitHub Pagesで配信する場合にも相対パスで動作する。PagesによるWeb配信はリポジトリ公開とは別の設定である。
...
- GitHubリポジトリは2026-09-08に本人が公開。配布ライセンスは未設定で、本人の決定後に付与する。
